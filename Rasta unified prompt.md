...
- تست ناپایدار گزارش‌شده‌ی نوبت قبل: در ۹ اجرای کامل پشت‌سرهم دیگه بازتولید نشد — زیر نظر. ✅ **حل شد ۲۰۲۶-۰۹-۲۶ (ادامه ۸)**: BookingFactory.
...
همون الگوی «تست ناپایدار زیر نظر» نشست‌های قبل؛ دفعه‌ی بعد خروجی هر اجرا در فایل ذخیره بشه تا اسمش پیدا بشه. ✅ **حل شد (ادامه ۸).**

### ۲۰۲۶-۰۹-۲۶ (ادامه ۵) — درگاه پارسیان، تسویه‌ی وندار، پروکسی‌های مورد اعتماد + پرامپت تست دستی

روی بچ Docker، ۴ کامیت کد + این سند. تحویل: `batch-2026-09-26-parsian-vandar/` (همراه `MANUAL_GATEWAY_TESTING_PROMPT.md`).

**۱) `fix(http): trust Cloudflare's edge as a proxy …`** — 🐞 **باگ واقعی، با probe بازتولید شد:** پشت Cloudflare همه‌ی مشتری‌هایی که از
یک لبه می‌اومدن یک سهمیه‌ی ورود مشترک داشتن (`throttle:auth` با کلید IP) — اولین تلاش ورود مشتری B با ۴۲۹ رد شد چون A سهمیه رو تموم
کرده بود؛ و آدرس‌ها (callback درگاه‌ها) همیشه `http://`.
**تصمیم (ابوالفضل گفت «انجامش بده»):** پیش‌فرض فقط رنج‌های رسمی Cloudflare (cloudflare.com/ips، ۱۵ IPv4 + ۷ IPv6)، هرگز `*`
(با `*` هر کسی با X-Forwarded-For جعلی محدودیت ورود رو دور می‌زنه). Symfony آی‌پی مشتری رو از سمت راست (اولین غیرمعتمد) برمی‌داره،
پس سرآیند جعلی بی‌اثره و سرور بدون Cloudflare دقیقاً مثل قبله. `TRUSTED_PROXIES` (کاما): IP/CIDR اضافه (مثلاً `172.16.0.0/12` برای
nginx میزبان جلوی Docker)، `*` یا `none`. `App\Http\Middleware\TrustProxies` جای middleware فریم‌ورک و config رو هنگام درخواست می‌خونه
(`TrustProxies::at()` در bootstrap قبل از config/env اجرا می‌شه). `TrustedProxiesTest` (۶) — بدون اصلاح ۵ تا fail.
⚠️ درس تست: درخواست وب throttle‌شده ۳۰۲ + پیام سشن می‌ده، نه ۴۲۹ — probe رو با `postJson` بزن.

**۲) `feat(payments): Parsian (PEC) direct bank gateway (SOAP) …`** — آخرین درگاه مستقیم مرحله‌ی ۲.
- منبع: صفحه‌ی راهنمای خودِ وب‌سرویس زنده (SalePaymentRequest: SOAP 1.1، namespace `https://pec.Shaparak.ir/NewIPGServices/Sale/SaleService`،
  SOAPAction `{ns}/SalePaymentRequest`) + Parbad، shetabit، pec_ir. PDF رسمی عمومی نبود؛ fetch سایت‌های pec/vandar با robots بسته‌ست.
- شروع SalePaymentRequest {LoginAccount=PIN, Amount ریال, OrderId=id تراکنش, CallBackUrl} → هدایت GET به `pec.shaparak.ir/NewIPG/?Token=`؛
  تایید ConfirmPayment با **توکن ذخیره‌شده‌ی خودمون**؛ Token/OrderId/Amount/TerminalNo بازگشت باید بخونن؛ کلیدهای بازگشت بدون حساسیت
  به حروف؛ رسید (توکن) با GatewayReceipt؛ ‎-1533/2 فقط با RRN بازگشت موفقه.
- پارسیان تراکنشِ تاییدشده رو برمی‌گردونه → «ساعت نوبت از دست رفت» **به کارت** (نه کیف پول). reconcile پنجره‌ی ۲۰–۵۰ دقیقه.
- ⚠️ تأییدنشده از سرویس زنده: namespace سرویس برگشت (`…/Reversal/ReversalService`، SOAP 1.2، از Parbad) و مهلت دقیق برگشت (‎-1549
  لاگ می‌شه). اشتباه بودنش فقط برگشت رو false می‌کنه → مسیر جایگزین.
- `ParsianGatewayTest` (۱۵). ⚠️ درس: در اولین نسخه ورودی کاتالوگ داخل آرایه‌ی `fields` ملت افتاده بود (پیدا کردن `"        ],\n"`
  با `"            ],\n"` هم جور درمیاد) — تست صفحه‌ی مدیریت گرفتش.

**۳) `feat(payouts): Vandar settlement driver …`** — مرحله‌ی ۳ تسویه، کنار زرین‌پال و زیبال.
- ✅ **واحد مبلغ حل شد: تومان** (حداقل ۵٬۰۰۰): متن مستند رسمی + نمونه‌ی رسمی (درخواست `amount: 5000` → پاسخ `amount: 50000` و
  `amount_toman: 5000`). حتی اگه روزی ریال بشه، تومان فرستادن فقط ۱/۱۰ واریز می‌کنه، هرگز ۱۰ برابر.
- `POST api.vandar.io/v3/business/{business}/settlement/store`؛ `track_id = mahru-wd-{id برداشت}` (یک برداشت دو تسویه نمی‌سازه).
- توکن ۵ روزه؛ `POST /v3/refreshtoken` **refresh_token جدید** هم می‌ده → هر دو ذخیره روی ردیف درگاه؛ `payouts:refresh-vandar-tokens`
  روزانه ۰۳:۳۰ + روی ۴۰۱ یک بار تمدید و تکرار؛ زیر قفل کش و اگه کس دیگه‌ای تازه تمدید کرده، همون استفاده می‌شه.
- 🐞 **خطر مالی (برای زیبال/زرین‌پال هم هست):** پاسخ تسویه بعد از فرستادن نرسه → قبلاً «ناموفق» و برگشت به کیف پول متخصص = احتمال
  پرداخت دوباره. حالا `PayoutResult::unknown` → برداشت در `processing` با `payment_details.needs_manual_check` می‌مونه، برگشت و رویدادی
  نیست، تکرار کور هم نیست؛ مدیر در داشبورد وندار می‌بینه و دستی تأیید/رد می‌کنه. زیبال و زرین‌پال هنوز این رو ندارن (قدم باز).
- `VandarPayoutTest` (۱۲). ⚠️ درس: import رویدادهای برداشت اشتباه بود (`App\Events\Withdrawal\{Approved,Rejected}\…` درسته) و
  `assertNotDispatched` روی کلاسِ ناموجود بی‌صدا پاس می‌شد.

**۴) پرامپت تست دستی** (`MANUAL_GATEWAY_TESTING_PROMPT.md`، خارج از ریپو): قواعد Claude برای جلسه (بازتولید با پاسخ واقعی قبل از
اصلاح، بدون نشر کلید)، آماده‌سازی سرور، سناریوهای مشترک (تست اتصال، پرداخت، انصراف، refresh، شارژ کیف پول، failover، کارمزد) و
مخصوص هر درگاه (تسویه‌ی واقعی زیبال/وندار با گزارش مبلغ ثبت‌شده؛ Settlement آسان‌پرداخت)، و قالب گزارش.

**بررسی:** Mutation — پارسیان: بدون claim ۴، بدون چک مبلغ/ترمینال ۱، تایید با توکن callback ۱ (با تست بازگشت بدون Token)، ‎-1533 بی RRN ۱،
بدون پنجره‌ی reconcile ۱؛ وندار: ریال ۲، track_id تصادفی ۴، ذخیره‌نشدن refresh ۲، بی‌قفل ۱، نامعلوم=ناموفق ۲، تکرار پس از پاسخ گم‌شده ۱؛
بدون شاخه‌ی unknown در job ۱. سوییت کامل **۱۳۸۷ passed / ۱ skipped**؛ Pint PASS (۸۱۴ فایل). وریفای روی کلون تازه با `git am`
(بچ ملت + Docker + همین بچ).

### ۲۰۲۶-۰۹-۲۶ (ادامه ۶) — «نتیجه‌ی نامعلوم» در همه‌ی درگاه‌ها (تسویه + تایید پرداخت)

روی بچ پارسیان/وندار، ۲ کامیت کد + این سند. تحویل: `batch-2026-09-26-unknown-outcome/`.

**دامنه:** درخواستی که پول جابه‌جا می‌کنه فرستاده شد ولی پاسخ نرسید.
| | تسویه | تایید پرداخت |
|---|---|---|
| زرین‌پال، زیبال | رفع شد | رفع شد |
| وندار | از قبل (دقیق‌تر شد) | رفع شد |
| آسان‌پرداخت | API تسویه نداره | رفع شد + باگ Settlement |
| سامان، ملت، پارسیان | API تسویه ندارن | از قبل درست (reconcile برگشت می‌زنه) |

**۱) `fix(payouts): a lost Zarinpal or Zibal payout answer is never refunded …`** — 🐞 پاسخ تسویه نمی‌رسید → «ناموفق» → برگشت به کیف پول
متخصص، در حالی که شاید واریز شده بود (پرداخت دوباره). حالا `unknown` → برداشت در `processing` با `needs_manual_check`.
`App\Payments\HttpFailure::neverSent()`: cURL 5/6/7/35/60 = درخواست هرگز از سرور خارج نشد → ناموفق ساده (قطعی درگاه صف بررسی دستی
نمی‌سازه)؛ بقیه (28 timeout، 52، 56، …) نامعلوم. وندار هم «فرستاده نشد» رو حالا ناموفق ساده می‌گیره.

**۲) `fix(payments): a lost verify answer at Zarinpal, Zibal, Vandar or Asan Pardakht is recovered by reconcile …`** — 🐞 پاسخ تایید گم می‌شد
→ «ناموفق» و لغو نوبت؛ اگر درگاه تایید کرده بود، مشتری پول داده بود و نه نوبت داشت نه بازگشت (فقط refresh تصادفی نجاتش می‌داد).
- driverها: فرستاده شد ولی بی‌پاسخ / ۵xx واقعی → `unanswered`. ⚠️ آسان‌پرداخت کدهای خطای منطقی خودش رو در بازه‌ی ۵xx می‌ده (۵۷۳ =
  امکان تایید نیست) — فقط ۵۰۰/۵۰۲/۵۰۳/۵۰۴ و ۵۷۱ (هنوز پردازش نشده) / ۵۷۲ (وضعیت نامشخص) نامعلوم‌اند.
- `UnansweredVerifyRecovery` در `payments:reconcile`، ۱۰ دقیقه (فرصت refresh مشتری) تا ۲۴ ساعت بعد از آخرین تلاش مشتری، همون
  `verify()` خود driver رو دوباره می‌پرسه — دقیقاً کار refresh، و تنها قراردادی که در همه‌ی driverها تست‌شده (معنای وضعیت‌های
  استعلام زیبال/وندار از منبع قابل‌تأیید پیدا نشد؛ استعلام زرین‌پال مستنده: VERIFIED/PAID/IN_BANK/FAILED/REVERSED):
  تایید شده بود → کل مبلغ (با کارمزد) به کیف پول مشتری، `refunded`، پیامک؛ شارژ کیف پول → خود شارژ (بدون کارمزد)، `paid`، پیامک
  `charge_recovered`. رد قطعی → بسته (پول تاییدنشده رو درگاه به کارت برمی‌گردونه). باز بی‌پاسخ → اجرای بعدی.
- هم‌زمانی: `failed → reconciling` اتمی؛ callback نوبت و شارژ برای `reconciling` «در حال بررسی، نتیجه با پیامک» می‌گن و دوباره
  تایید نمی‌کنن (وگرنه هم نوبت ثبت می‌شد هم پول برمی‌گشت).
- ⚠️ بده‌بستان: اگه اولین تایید اصلاً به درگاه نرسیده بود، این دوباره‌پرسیدن پول رو می‌گیره و مشتری به‌جای برگشت به کارت، اعتبار
  کیف پول می‌گیره — همون نتیجه‌ای که refresh خودش داشت. استرداد به کارت از پنل درگاه دستی ممکنه.
- 🐞 باگ جدا (آسان‌پرداخت): Verify موفق ولی Settlement خطا → «ناموفق» و لغو نوبت با پولِ گرفته‌شده. حالا موفق + `settlement_failed` + لاگ خطا.

**تست:** `UnknownPayoutOutcomeTest` (۳)، `UnknownVerifyOutcomeTest` (۳)، `UnansweredVerifyRecoveryTest` (۷، مسیر کامل نوبت زیبال)، و به‌روزرسانی
`VandarPayoutTest` (گم‌شدن پاسخ با timeout واقعی جعل می‌شه؛ `Http::failedConnection()` پیش‌فرض خطای DNS یعنی «فرستاده نشد» است).
Mutation: همه‌ی ۸ مورد گرفته شدن (قفل هم‌زمانی اول زنده موند → تست «دو اجرای هم‌پوشان» اضافه شد). سوییت کامل **۱۴۰۱ passed / ۱ skipped**.
⚠️ درس: `rm` بعد از اسکریپتی که شکست خورد اجرا شد و یک فایل تست ثبت‌نشده پاک شد — دوباره ساخته و با همون تعداد assertion و
mutationها وریفای شد. دستورهای پاک‌کردن رو با `&&` به موفقیت مرحله‌ی قبل گره بزن.

### ۲۰۲۶-۰۹-۲۶ (ادامه ۷) — «نیاز به بررسی انسانی» در پنل مدیریت + نشت فهرست برداشت/کیف پول بین سالن‌ها

روی بچ unknown-outcome، ۲ کامیت کد + این سند. تحویل: `batch-2026-09-26-attention/`.

**۱) `fix(admin): the withdrawal and specialist-wallet lists showed other salons' records`** — 🔴 **نشت داده‌ی بین سالن‌ها، با probe
بازتولید شد:** مالک سالن A در `/admin/wallet/withdrawals` ردیف برداشت سالن B (مبلغ، تاریخ، شناسه) رو می‌دید و درخواست B در کارت
«در انتظار» A شمرده می‌شد؛ صفحه‌ی کیف پول‌ها هم فهرست و جمع موجودی/درآمد/برداشت همه‌ی سالن‌ها رو نشون می‌داد. جدول‌های
`withdrawal_requests` و `specialist_wallets` ستون salon_id و مدل‌هاشون scope ندارن؛ فقط صفحه‌ی جزئیات چک مالکیت داشت
(`CrossSalonImplicitBindingTest`). حالا repositoryها فهرست و آمار رو با `whereHas('specialist')` (scope سراسری BelongsToSalon متخصص)
می‌سازن. `WalletListSalonScopeTest` (۲) — بدون اصلاح هر دو fail.
⚠️ درس کلی: هر جدول بدون salon_id که از طریق رابطه به سالن وصله، فهرست/آمارش باید جدا بررسی بشه؛ تست binding فقط جزئیات رو می‌پوشونه.

**۲) `feat(admin): payments and payouts that automation gave up on are visible and resolvable`**
- پرچم‌ها ستون واقعی و index‌دار (نه کلید داخل json — مقایسه‌ی boolean در json بین SQLite/MySQL/MariaDB یکسان نیست):
  - `withdrawal_requests.needs_manual_check`: `ProcessWithdrawalJob` روی نتیجه‌ی نامعلوم تسویه؛ `markAsCompleted()`/`markAsFailed()` پاکش
    می‌کنن. تایید دستی سابقه‌ی تلاش نامعلوم رو در `payment_details.unknown_payout` نگه می‌داره.
  - `payment_transactions.needs_attention`: گام جدید `payments:reconcile` — failed و هنوز unanswered بعد از بازه‌ی درگاه (مستقیم: پایان
    بازه‌ی برگشت — شامل ملتِ «settle شده ولی ناموفق ثبت‌شده» و پارسیانِ «مهلت برگشت گذشته»؛ غیرمستقیم: ۲۴ ساعت)، یا گیرکرده در
    reconciling/reversing. فقط یک هفته بعد از هر بازه؛ updated_at دست نمی‌خوره. همچنین آسان‌پرداختِ Verify‌شده ولی Settlement‌نشده.
- **صفحه‌ی «پرداخت‌های نیازمند بررسی»** (`admin/payment-attention`، فقط owner): دلیل، شناسه‌ی درگاه برای جست‌وجو در پنل، مشتری، مبلغ.
  دو کار: «واریز به کیف پول مشتری» (همون مسیر بازیابی خودکار، با پیامک؛ ادعای اتمی → دو کلیک هم‌زمان یک بار واریز) یا «رسیدگی شد»
  با یادداشت اجباری (بدون جابه‌جایی پول؛ ردیف گیرکرده به failed برمی‌گرده). ثبت در `verify_response.manual_resolution`.
- برداشت‌ها: فیلتر و بنر «نیاز به بررسی دستی»، نشان روی ردیف، توضیح در صفحه‌ی جزئیات. نشان تعداد در منو و هشدار داشبورد برای owner
  (`AttentionCounts` — عمداً بدون حافظه؛ نمونه‌ی ماندگار عدد کهنه نشون می‌داد).
- `UnansweredVerifyRecovery::credit()` بین مسیر خودکار و دستی مشترکه (`recovery_via`).
- `PaymentAttentionTest` (۱۰). Mutation: هر ۷ خط محافظ یک تست رو fail می‌کنن (دو مورد اول زنده موندن → تست «دو کلیک هم‌زمان» و
  «برداشت سالن دیگه در شمارش» اضافه شد). ⚠️ درس تست: approve/reject برداشت `PUT` است؛ `post` → ۴۰۵ بی‌صدا.

**بررسی:** سوییت کامل **۱۴۱۳ passed / ۱ skipped**؛ Pint PASS (۸۲۴ فایل). MariaDB 10.11: ۲۲۶ تست پرداخت/job/scope ادمین PASS؛ migration جدید
بالا و rollback روی MariaDB. وریفای روی کلون تازه با `git am` (همه‌ی بچ‌ها به ترتیب).

### ۲۰۲۶-۰۹-۲۶ (ادامه ۸) — ✅ ریشه‌ی «تست ناپایدار» چند نشست پیدا و رفع شد

در همین بچ (`batch-2026-09-26-attention/`)، کامیت `test: factory bookings could fall inside the cancellation-fee window …`.
- **تست:** `BookingServiceTest::test_cancel_booking_on_a_paid_booking_triggers_the_wallet_refund` — گاهی بازپرداخت ۴۸٬۰۰۰ به‌جای ۶۰٬۰۰۰
  (یعنی جریمه‌ی ۲۰٪ لغو مشتری). با ذخیره‌ی خروجی هر اجرا اسمش پیدا شد.
- **ریشه در `BookingFactory`، نه کد:** تاریخ از «+۱ روز» کشیده می‌شد و بعد ساعت تصادفی ۹ تا ۱۷ روش گذاشته می‌شد. اگه تاریخ فردا
  می‌افتاد و ساعت زودتر از ساعت فعلی بود، نوبت کمتر از ۲۴ ساعت بعد بود → داخل `cancellation_before_hours` (۲۴) → جریمه. احتمالش به
  ساعت اجرای سوییت بستگی داشت (~۰٫۴۵٪ هر قرعه ساعت ۱۷:۳۰، صفر صبح زود). اثبات قطعی: همون تست با نوبت ۲۳ ساعت بعد دقیقاً ۴۸٬۰۰۰ می‌ده.
- **رفع در factory (به نفع همه‌ی تست‌ها):** از `now()->addDays(2)`، و با مرزهای Carbon به‌جای رشته‌ی `'+1 day'` — Faker رشته‌ی نسبی رو
  با ساعت واقعی سیستم حساب می‌کنه، پس factory `travelTo()` تست‌ها رو نادیده می‌گرفت.
- `BookingFactoryTimeTest`: ساعت ۱۷:۳۰ (بدترین حالت)، Faker با seed، ۱۰۰۰ قرعه → با بازه‌ی قدیمی هر بار fail (نزدیک‌ترین نوبت ۱۷٫۵ ساعت)،
  با اصلاح هر بار pass.
- ⚠️ درس: نسخه‌ی اول این تست با `travelTo(23:30)` قبل از اصلاح هم پاس می‌شد (دو دلیل: Faker ساعت واقعی رو می‌خوند، و ساعت ۲۳:۳۰ بدترین
...
### ۲۰۲۶-۰۹-۲۷ — ۴ باگ MySQL-only + ممیزی کامل جداسازی سالن‌ها (سه پنل) + تصمیم‌های «مختص هر سالن» + باگ پرداخت ناموفق

۱۷ کامیت روی `dd1d3d6` (۱۵ رفع/قابلیت + تست ناپایدار + این سند)؛ هر رفع در کامیت جدا، هر کدوم با تست رگرسیونی که **قبل از رفع شکست می‌خورد** (بازتولید با تست موقت، بعد رفع).
در پایان کل سوییت: **SQLite ۱۴۷۲ (۲ skip)، MariaDB 10.11 ۱۴۷۲ (۱ skip)، بدون شکست** — هم روی کلون کار، هم روی کلون تازه بعد از `git am`.
۵ migration جدید روی هر دو دیتابیس بالا و rollback شدن.

**بخش ۱ — ۴ باگ MySQL-only (قدم باز قبلی):**
- `668b5a9` تغییر زمان نوبت (۵۰۰ / redirect به ریشه) و امتیاز وفاداری بعد از امتیازدادن — **یک علت**: کانال database سفارشی
  (`AppServiceProvider`) و `UserNotification::creating` شناسه‌ی `notifiable` رو در `user_notifications.user_id` (FK به users) می‌نوشتن؛
  برای اعلانی که به مدل `Specialist` می‌ره یعنی **شناسه‌ی متخصص**. MySQL: کاربر هم‌شماره نبود → FK 1452 (داخل transaction تغییر زمان →
  rollback)؛ بود → اعلان به کاربر بی‌ربط (شاید سالن دیگه). SQLite پنهانش می‌کرد چون شمارنده‌ی AUTOINCREMENT با rollback هر تست برمی‌گرده
  (MariaDB نه → شکست وابسته به ترتیب اجرا). رفع: `UserNotification::ownerUserId()` + ستون nullable.
- `22ca174` `two_factor_code` varchar(6) در برابر `TWO_FACTOR_CODE_LENGTH` بی‌سقف (+ سرریز random_int بالای ۱۸ رقم) → ستون ۱۰ و سقف ۱۰.

**بخش ۲ — ممیزی نشت (روش):** دو probe موقت (کامیت نشدن): (۱) پر کردن دو سالن با رشته‌های نشانه‌دار در همه‌ی موجودیت‌ها و خزیدن
**همه‌ی مسیرهای GET** هر پنل به‌عنوان مالک/متخصص/مشتری سالن الف + IDOR با شناسه‌های کاربر دیگه‌ی همون سالن و سالن دیگه؛ (۲) probe
**تفاضلی** آمار: snapshot همه‌ی داده‌ی view/JSON/فایل هر صفحه با یک سالن، اضافه‌کردن سالن شلوغ دوم، snapshot دوباره، مقایسه —
شامل خروجی‌های Excel (همه‌ی شیت‌ها، سلول‌به‌سلول) و PDF که مثل worker (بدون سالن) اجرا شدن.

نشت‌های پیدا و رفع‌شده (همه با probe بازتولید شدن):
- `9c1fccf` اعلان‌های مدیریت (نوبت جدید، پرداخت، برداشت، نظر منفی) به **همه‌ی مدیرهای همه‌ی سالن‌ها** می‌رفت —
  `getAdminRecipients()` بدون سالن صدا زده می‌شد و `where(is_admin) OR …` بی‌گروه بود.
- `9aa9690` فهرست و JSON «در انتظار» مرخصی‌ها.
- `5f332b6` کل ماژول نظرات: فهرست، شمارنده‌ها، آمار، سطل زباله، جزئیات، تایید/رد/ویژه/حذف و **حذف دائمی نظر سالن دیگه** →
  trait جدید `BelongsToSalonThroughSpecialist` (برای جدول‌های بدون salon_id که از specialist_id به سالن وصلن).
- `b9603fe` بخش امنیت (رویدادها، فهرست کاربران با تلفن، شمارنده‌ها) → `UserRepository::querySalonMembers()` = «کاربرهای این سالن»:
  مشتری با salon_id، ادمین در salon_admins، کاربرِ متخصص‌های سالن (کارمندها salon_id=null دارن).
- `0f0cd0f` جست‌وجوی مدیریت، انتخاب‌گر مشتری در ویرایش نوبت و کد تخفیف، و اعتبارسنجی `user_id` در ثبت/ویرایش نوبت و کد تخفیف
  (`App\Rules\CustomerOfCurrentSalon`). `BookingFactory` حالا مشتری همین سالن رو برمی‌داره، نه هر کاربر تصادفی.
- `15c6efb` مدیریت امتیاز وفاداری: جست‌وجو، `?user_id` هر کاربر، **افزودن/کسر امتیاز مشتری هر سالن**، شمارنده‌های داشبورد وفاداری.
- `693ba81` شمارنده‌ی «کاربران» داشبورد (صفحه + `dashboard/data`) — تنها عددی که در probe تفاضلی تغییر کرد.

**تصمیم‌های ابوالفضل (۲۰۲۶-۰۹-۲۷) و پیاده‌سازی:**
1. `22bc828` جوایز وفاداری مال هر سالن: `rewards.salon_id` (nullable) + `BelongsToSalon` + چک مالکیت در همه‌ی اکشن‌های `{reward}`.
2. `f1df57a` نقش‌ها مختص هر سالن: `roles.salon_id` (null = نقش سیستمی که کد با نامش چک می‌کنه — فقط سوپرادمین تغییرش می‌ده؛
   سالن فقط می‌بینه و تخصیص می‌ده)؛ نام نقش سالن یکتا در سالن و **نمی‌تونه نام نقش سیستمی باشه** (hasRole با نام مقایسه می‌کنه —
   نقش سالنی به نام super-admin خطرناک بود)؛ تخصیص/حذف فقط برای اعضای سالن. **مجوزها** یک کاتالوگ مشترک موندن (نام‌هایی که کد
   چک می‌کنه)، ساخت/ویرایش/حذف فقط سوپرادمین؛ جدایی سالن‌ها از این راهه که هر سالن تعیین می‌کنه نقش‌های خودش چه مجوزهایی دارن.
3. `2574173` تنظیمات اطلاع‌رسانی و تنظیمات امنیتی مختص هر سالن. اعلان‌ها اغلب در صف ساخته می‌شن (بدون CurrentSalon) → تنظیمات
   از سالنِ **گیرنده** خونده می‌شه (`App\Support\SalonOfNotifiable`، همه‌ی ۱۸ `via()`). ⚠️ rollback این migration روی MariaDB اول
   شکست خورد (1553: ایندکس یکتا پشتیبان FK است) — `down()` اول FK رو حذف می‌کنه.
4. `faa2183` دستور یک‌باره `tenancy:repair-legacy-rows [--rewards-salon=<slug>] [--dry-run]`: اصلاح `user_id` اعلان‌های قدیمی
   متخصص + سپردن جوایز بدون سالن به یک سالن.

**باگ‌های جانبی:**
- `c9b266d` 🔴 **پول:** «انصراف» مشتری در صفحه‌ی بانک (رایج‌ترین شکست) نتیجه‌ی بدون `booking_id` برمی‌گردوند → callback خطا →
  کل مسیر شکست رد می‌شد: **سهم کیف پولِ پرداخت ترکیبی برنمی‌گشت** (تست: موجودی ۰ به‌جای ۲۰٬۰۰۰)، نوبت آزاد نمی‌شد. و صفحه‌ی
  `bookings.failed` چون هیچ redirectـی `booking_id` رو flash نمی‌کرد، **برای هر پرداخت ناموفق ۵۰۰ بود**. تست‌های قبلی فقط redirect
  رو چک می‌کردن، نه خود صفحه.
- `bb49b13` حذف ۱۵ مسیر مشتری که به متدهای ناموجود اشاره می‌کردن (بعد از بررسی: هیچ صفحه/JS لینکشون نمی‌داد؛ معادل کارکننده
  برای بقیه موجوده؛ علاقه‌مندی/مقایسه/… هرگز ساخته نشده بودن). اسکن هر ۴۴۲ مسیر: هیچ مسیری به متد ناموجود نیست؛ `route:cache` سالم.
- تست ناپایدار: `SendBookingRemindersTest::…just_inside_the_window_boundaries…` یک بار در اجرای کامل MariaDB روی کلون تازه شکست
  خورد (نوبت دقیقاً روی مرز «الان+۵۵»، دستور «الان» خودش رو کمی بعد حساب می‌کنه؛ تیک یک ثانیه → بیرون از بازه). با `sleep(1)` موقت
  قطعی بازتولید شد؛ کامیت جدا `freezeTime()`.
- صفحه‌ی 2FA با `two_factor_enabled = null`: فقط با مدل تست ذخیره‌نشده رخ می‌ده (ستون NOT NULL DEFAULT false) — باگ واقعی نیست.

**بررسی‌شده و سالم:** پنل متخصص (همه‌ی صفحه‌ها، دسترسی به نوبت/نظر/اعلان متخصص دیگه همون سالن و سالن دیگه → ۴۰۳/۴۰۴/redirect،
گزارش و Excel خودش)؛ پنل مشتری و API (نوبت/تغییر زمان/پرداخت/تراکنش کیف پول مشتری دیگه → ۴۰۳/۴۰۴)؛ مدیریت: کیف پول، برداشت‌ها،
خدمات، متخصص‌ها، نوبت‌ها، گزارش‌ها (امروز/هفته/ماه)، خروجی‌ها، تیکت‌ها، نیاز به بررسی.

**درس‌ها:**
- SQLite شمارنده‌ی AUTOINCREMENT رو با rollback تست برمی‌گردونه؛ باگی که به «هم‌شماره بودن» دو جدول تکیه داره فقط روی MySQL دیده می‌شه.
  تست رگرسیون: یک شناسه‌ی دور (مثلاً ۹۰۰۰۰۱) بدید.
- route model binding قبل از ست‌شدن CurrentSalon انجام می‌شه → global scope به‌تنهایی جلوی IDOR رو نمی‌گیره؛ هر اکشن `{model}` مدیریت
  `ensureSalonOwnership()` لازم داره.
...
- اگر nginx/certbot روی خود سرور جلوی برنامه است: `TRUSTED_PROXIES=cloudflare,<آدرس آن پروکسی>` (پیش‌فرض فقط Cloudflare).
