...
@section('content')
    <div class="container px-6 mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold" style="color:var(--admin-text)">ویرایش: {{ $role->label }}</h1>
                <p class="text-sm mt-1" style="color:var(--admin-text-dim)">ویرایش اطلاعات و دسترسی‌های نقش</p>
            </div>
        ...
        <div class="rounded-xl p-6" style="background:var(--admin-surface);border:1px solid var(--admin-border)">
            <form action="{{ route('admin.roles.update', $role) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:var(--admin-text-dim)">نام فنی (انگلیسی)</label>
                        <input type="text" name="name" value="{{ old('name', $role->name) }}" dir="ltr" required
                        ...
                        <p class="text-xs mt-1" style="color:var(--admin-text-light)">نام انگلیسی برای استفاده در کد</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1" style="color:var(--admin-text-dim)">عنوان نمایشی (فارسی)</label>
