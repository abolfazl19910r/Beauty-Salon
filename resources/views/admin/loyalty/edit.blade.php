    ...
    <div class="container px-6 mx-auto">
        ...
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold flex items-center gap-2" style="color:var(--admin-text)">
                <svg class="w-6 h-6" style="color:var(--admin-accent)" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                ویرایش: {{ $reward->title }}
            </h1>
            ...
            <form action="{{ route('admin.loyalty.rewards.update', $reward) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="number" name="required_points" value="{{ $reward->required_points }}" required
                               class="w-full rounded-lg px-3 py-2 text-sm"
                               style="border:1px solid var(--admin-border);background:var(--admin-bg);color:var(--admin-text)">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" rows="3"
                                  class="w-full rounded-lg px-3 py-2 text-sm"
                                  style="border:1px solid var(--admin-border);background:var(--admin-bg);color:var(--admin-text)">{{ $reward->description }}</textarea>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_amount" id="discount_amount" value="{{ $reward->discount_amount }}"
                               class="w-full rounded-lg px-3 py-2 text-sm"
                               style="border:1px solid var(--admin-border);background:var(--admin-bg);color:var(--admin-text)">
                    ...
                    <div>
                        ...
                        <input type="number" name="max_uses" value="{{ $reward->max_uses }}" min="{{ $reward->used_count }}"
                               class="w-full rounded-lg px-3 py-2 text-sm"
                               style="border:1px solid var(--admin-border);background:var(--admin-bg);color:var(--admin-text)">
                    </div>

                    <div class="flex items-center gap-2 mt-2">
                        <input type="checkbox" name="is_active" id="is_active" {{ $reward->is_active ? 'checked' : '' }}
