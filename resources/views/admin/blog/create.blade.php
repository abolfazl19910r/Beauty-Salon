...
@section('content')
    <div class="fade-in">
        <div class="flex justify-between items-center mb-5">
            <h1 class="text-xl font-bold" style="color:var(--admin-text);">ایجاد مقاله جدید</h1>
                    ...
                    <div class="rounded-xl p-5" style="background:var(--admin-surface); border:1px solid var(--admin-border);">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="title" value="{{ old('title') }}" class="form-input" required placeholder="عنوان مقاله را وارد کنید">
                                @error('title') <p class="form-error">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="form-label">چکیده (اختیاری)</label>
                                <textarea name="excerpt" rows="3" class="form-textarea" placeholder="خلاصه کوتاه مقاله...">{{ old('excerpt') }}</textarea>
                                @error('excerpt') <p class="form-error">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label class="form-label">محتوا <span style="color:#DC2626;">*</span></label>
                                <textarea name="content" rows="10" class="form-textarea" required placeholder="متن کامل مقاله را اینجا بنویسید...">{{ old('content') }}</textarea>
                    ...
                    <div class="rounded-xl p-5" style="background:var(--admin-surface); border:1px solid var(--admin-border);">
                        ...
                        <label for="blog-image" class="block border-2 border-dashed rounded-lg p-6 text-center cursor-pointer transition-colors"
                            ...
                            <div id="blog-preview-container" class="hidden mb-3">
                                <img id="blog-image-preview" src="" class="w-32 h-24 object-cover rounded-lg mx-auto">
                            </div>
                            <svg class="w-8 h-8 mx-auto mb-2" id="blog-upload-icon" style="color:var(--admin-text-light);" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/>
                            </svg>
                            <p class="text-sm font-medium" style="color:var(--admin-accent);" id="blog-file-name">انتخاب تصویر</p>
                    ...
                    <div class="rounded-xl p-5" style="background:var(--admin-surface); border:1px solid var(--admin-border);">
                        ...
                        <div class="space-y-4">
                            ...
                            <label class="flex items-center gap-2 p-3 rounded-lg cursor-pointer"
                                ...
                                <span class="text-sm" style="color:var(--admin-text);">همین الان منتشر شود</span>
                            </label>
                        </div>
                    </div>
