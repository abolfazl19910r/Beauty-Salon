...
@section('content')
    <div class="fade-in max-w-2xl">
        <h1 class="text-xl font-bold mb-5" style="color:var(--admin-text);">کد تخفیف جدید</h1>
        ...
        <form action="{{ route('admin.discount-codes.store') }}" method="POST"
            ...
            <div>
                <label class="block text-sm mb-1" style="color:var(--admin-text);">کد تخفیف</label>
                <input type="text" name="code" id="code" value="{{ old('code') }}" required
                       placeholder="مثلاً SUMMER1405" dir="ltr"
                       class="w-full px-3 py-2 rounded-lg" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                <p class="text-xs mt-1" style="color:var(--admin-text-dim);">کد در زمان ذخیره خودکار به حروف بزرگ تبدیل می‌شود.</p>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm mb-1" style="color:var(--admin-text);">نوع تخفیف</label>
                    <select name="type" id="type" required
                            class="w-full px-3 py-2 rounded-lg" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                        <option value="percentage" @selected(old('type', 'percentage') === 'percentage')>درصدی</option>
                        <option value="fixed" @selected(old('type') === 'fixed')>مبلغ ثابت</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm mb-1" style="color:var(--admin-text);">
                        مقدار <span id="amount-unit" style="color:var(--admin-text-dim);">(درصد)</span>
                    </label>
            ...
            <div>
                ...
                <input type="number" name="max_amount" id="max_amount" min="0" value="{{ old('max_amount') }}"
                       class="w-full px-3 py-2 rounded-lg" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                <p class="text-xs mt-1" style="color:var(--admin-text-dim);">
                    برای تخفیف درصدی، مبلغ نهایی تخفیف هیچ‌وقت از این سقف بیشتر نمی‌شود. برای مبلغ ثابت معمولاً لازم نیست.
                </p>
            </div>

            <div class="rounded-lg p-4" style="background:var(--admin-bg); border:1px solid var(--admin-border);">
                <p class="text-sm font-medium mb-2" style="color:var(--admin-text);">پیش‌نمایش تخفیف</p>
                <div class="flex items-center gap-2 mb-3">
                    <label class="text-xs whitespace-nowrap" style="color:var(--admin-text-dim);">روی مبلغ نمونه (تومان)</label>
                    <input type="number" id="preview_base_amount" value="1000000" min="0"
                           class="w-full px-3 py-1.5 rounded-lg text-sm" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                    <button type="button" id="preview-btn"
                            class="whitespace-nowrap px-3 py-1.5 rounded-lg text-xs text-white"
                            style="background-color: var(--admin-accent);">محاسبه</button>
                </div>
                <div id="preview-result" class="text-sm" style="color:var(--admin-text-dim);">
                    برای دیدن پیش‌نمایش، مقادیر بالا را وارد کنید و «محاسبه» را بزنید.
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="jcal-wrapper">
                        ...
                        <input type="hidden" name="expires_at" id="expires-hidden" value="{{ old('expires_at') }}">
                        <div class="jcal-popup" id="expires-popup"></div>
                    </div>
                    {{-- No separate time picker: an expiry date is treated as "valid through the end of
                         that day" (23:59:59), unlike admin/announcements which needs a precise
                         publish/expire time-of-day. --}}
                </div>
            </div>

            <div>
                <label class="block text-sm mb-1" style="color:var(--admin-text);">اختصاص به یک کاربر خاص (اختیاری)</label>
                <select name="user_id" class="w-full px-3 py-2 rounded-lg" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                    <option value="">— برای همه‌ی کاربران —</option>
    ...
    <script>
        (function() {
            ...
            function syncAmountUnit() {
                amountUnit.textContent = typeSelect.value === 'percentage' ? '(درصد)' : '(تومان)';
            }
            typeSelect.addEventListener('change', syncAmountUnit);
            syncAmountUnit();

            // ---- Live preview: computed server-side via DiscountCalculator (route:
            // admin.discount-codes.preview), not reimplemented here in JS. ----
            ...
            previewBtn.addEventListener('click', function() {
                const params = new URLSearchParams({
                    ...
                    base_amount: document.getElementById('preview_base_amount').value || '0',
                });

                previewResult.textContent = 'در حال محاسبه...';

                fetch(`{{ route('admin.discount-codes.preview') }}?${params.toString()}`, {
                    ...
                    .then(function(data) {
                        const discount = Number(data.discount_amount).toLocaleString('fa-IR');
                        const final = Number(data.final_amount).toLocaleString('fa-IR');
                        previewResult.innerHTML =
                            'مبلغ تخفیف: <strong class="persian-number">' + discount + '</strong> تومان — ' +
                            'مبلغ نهایی: <strong class="persian-number">' + final + '</strong> تومان';
                    })
                    .catch(function() {
                        previewResult.textContent = 'محاسبه پیش‌نمایش با خطا مواجه شد. مقادیر وارد شده را بررسی کنید.';
                    });
            });

            /* ── jcal (عیناً از admin/announcements/create.blade.php، پایه‌ی تاریخ خودکفای پروژه) ── */
