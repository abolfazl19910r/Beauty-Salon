...
@section('content')
    <div class="fade-in max-w-2xl">
        <h1 class="text-xl font-bold mb-5" style="color:var(--admin-text);">اطلاعیه جدید</h1>
        ...
        <form action="{{ route('admin.announcements.store') }}" method="POST"
            ...
            <div>
                ...
                <textarea name="content" rows="4" required
                          class="w-full px-3 py-2 rounded-lg" style="border:1px solid var(--admin-border); color:var(--admin-text);">{{ old('content') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="type" required class="w-full px-3 py-2 rounded-lg" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                        <option value="general" @selected(old('type') === 'general')>عمومی</option>
                        <option value="maintenance" @selected(old('type') === 'maintenance')>تعمیرات</option>
                        <option value="promotion" @selected(old('type') === 'promotion')>تبلیغاتی</option>
                    </select>
                </div>
                <div>
                    <label class="block text-sm mb-1" style="color:var(--admin-text);">اولویت</label>
                    <input type="number" name="priority" min="0" value="{{ old('priority', 0) }}" required
                           class="w-full px-3 py-2 rounded-lg" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <div class="flex gap-2">
                        <div class="jcal-wrapper flex-1">
                            ...
                            <input type="hidden" name="published_at" id="published-hidden" value="{{ old('published_at') }}">
                            <div class="jcal-popup" id="published-popup"></div>
                        </div>
                        <input type="time" id="published-time" value="00:00"
                ...
                <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        <input type="time" id="expires-time" value="00:00"
                               class="w-24 rounded-lg px-2 py-2 text-sm" style="border:1px solid var(--admin-border); color:var(--admin-text);">
                    </div>
                </div>
            </div>
...
@push('scripts')
    <script>
        /* ── jcal (عیناً از admin/reports/index.blade.php، پایه‌ی تاریخ خودکفای پروژه) ── */
        (function(){
