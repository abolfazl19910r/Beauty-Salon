...
<x-guest-layout>
    ...
    <form method="POST" action="{{ route('salon.login') }}" class="space-y-5">
        @csrf
        ...
        <div class="flex items-center justify-between text-sm">
            ...
            @if (Route::has('salon.password.request'))
                <a href="{{ route('salon.password.request') }}" class="text-[#E6CD8A] hover:text-[#C9A24B] transition-colors">
                    فراموشی رمز عبور
                </a>
            @endif
        </div>
        ...
        <p class="text-center text-sm text-[#F8F3E9]/60">
            حساب کاربری ندارید؟
            <a href="{{ route('salon.register') }}" class="text-[#E6CD8A] hover:text-[#C9A24B] transition-colors font-medium mr-1">
                ثبت نام کنید
            </a>
        </p>
    </form>
