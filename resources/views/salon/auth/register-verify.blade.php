<x-guest-layout>
    <div class="text-center mb-8">
        ...
        <h2 class="text-2xl font-bold" style="font-family:'Noto Naskh Arabic','Vazirmatn',serif; color:#E6CD8A">تایید شماره موبایل</h2>
        <p class="text-sm text-[#F8F3E9]/60 mt-1">کد تایید به شماره موبایل شما ارسال شد</p>
        @if(isset($user))
            <p class="text-[#E6CD8A] font-medium mt-2 tracking-widest persian-number" dir="ltr">{{ $user->phone }}</p>
        @endif
    </div>
    ...
    <form method="POST" action="{{ route('salon.register.verify') }}" class="space-y-5">
        @csrf
    ...
    <div class="mt-5 flex items-center justify-between text-sm">
        <form method="POST" action="{{ route('salon.register.resend') }}" class="inline">
            @csrf
        ...
        <a href="{{ route('salon.register') }}" class="text-[#F8F3E9]/50 hover:text-[#F8F3E9] transition-colors">
            بازگشت به ثبت نام
        </a>
    </div>
    ...
    <div class="mt-5 flex items-start gap-2 bg-[#C9A24B]/10 border border-[#C9A24B]/20 rounded-lg px-4 py-3 text-sm text-[#F8F3E9]/70">
        ...
        خوش آمدید! با تایید شماره موبایل می‌توانید از تمام خدمات {{ $salon->name }} استفاده کنید.
    </div>
