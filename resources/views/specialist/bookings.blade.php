                            ...
                            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-1">
                                        ...
                                        @if($status == 'pending')
                                            <span class="text-[10px] bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full">در انتظار تایید</span>
                                        @elseif($status == 'confirmed')
                                            <span class="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">تایید شده</span>
                                        @endif
                                    </div>
                                    <div class="text-sm text-gray-600 space-y-1">
                                        <p>مشتری: {{ $booking->user->name }} | <span class="dir-ltr">{{ $booking->user->phone }}</span></p>
                                        <p>زمان: {{ verta($booking->booking_time)->format('Y/m/d ساعت H:i') }}</p>
                                        <p>وضعیت مالی:
                                            @if($booking->payment_status == 'paid') <span class="text-green-600 font-medium">پرداخت شده</span>
                                            @elseif($booking->payment_status == 'pending_payment') <span class="text-yellow-600 font-medium">در انتظار پرداخت</span>
                                            @else <span class="text-red-500 font-medium">پرداخت نشده</span>
                                            @endif
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('specialist.bookings.show', $booking->id) }}" class="bg-blue-50 text-blue-600 p-2 rounded hover:bg-blue-100 transition" title="مشاهده جزئیات">
        ...
        <div class="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <form id="confirmForm" method="POST">
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 bg-green-600 text-white py-2 rounded-lg font-bold">بله، تایید شود</button>
                    <button type="button" onclick="hideConfirmModal()" class="flex-1 bg-gray-100 py-2 rounded-lg">انصراف</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="cancelListModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <form id="cancelListForm" method="POST">
                ...
                <div class="flex gap-3">
                    <button type="submit" class="flex-1 bg-red-600 text-white py-2 rounded-lg font-bold">لغو نوبت</button>
                    <button type="button" onclick="hideCancelModal()" class="flex-1 bg-gray-100 py-2 rounded-lg">انصراف</button>
                </div>
            </form>
        </div>
    </div>
