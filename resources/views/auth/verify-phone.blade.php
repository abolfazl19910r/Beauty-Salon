...
@section('content')
    <div class="max-w-md mx-auto fade-in">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-[#E6CD8A]" style="font-family:'Noto Naskh Arabic','Vazirmatn',serif">تایید شماره موبایل</h1>
            <p class="text-sm text-[#F8F3E9]/55 mt-1">برای دسترسی به این بخش، ابتدا کد ۶ رقمی ارسال‌شده به شماره‌ی زیر را وارد کنید</p>
            <p class="text-[#E6CD8A] font-medium mt-2 tracking-widest persian-number" dir="ltr">{{ auth()->user()->phone }}</p>
        </div>
        ...
        <div class="mt-4 text-center">
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="text-sm text-[#F8F3E9]/50 hover:text-[#E6CD8A] transition-colors">
                    خروج از حساب
                </button>
            </form>
        </div>
    ...
    <script>
        const verifyUrl = '{{ route('verification.verify') }}';
        const resendUrl = '{{ route('verification.resend') }}';
        const fallbackUrl = '{{ url('/dashboard') }}';
